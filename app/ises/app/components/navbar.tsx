...
export default () => {
    ...
    return (
        <nav className="bg-indigo-900 py-2 shadow-md">
            <h1 className="text-indigo-50 rounded-md px-3 py-2 text-2xl font-bold inline-block">
                ises
            </h1>
            <span className="space-y-1 px-2 pb-3 pt-2">
                ...
                <span>
                    ...
                    <Menu as="div" className="relative inline-block text-left">
                        <div>
                            <Menu.Button className="inline-flex w-full justify-center gap-x-1.5 rounded-md dark:bg-slate-700 dark:hover:bg-slate-600 dark:text-gray-200 bg-indigo-100 px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                                {ip}
                                <svg
                        ...
                        >
                            <Menu.Items className="dark:bg-slate-700 dark:text-gray-200 absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                <div className="py-1">
                                    {ips.map((ip_) => (
                                        <Menu.Item key={ip_}>
                                            {({ active }) => (
                                                <Link
                                                    ...
                                                    className={classNames(
                                                        active
                                                            ? "bg-indigo-100 text-indigo-900 dark:bg-slate-600 dark:text-indigo-400"
                                                            : ip == ip_
                                                                ? "bg-indigo-200 dark:bg-slate-600"
                    ...
                    {loggedIn && dropdown != null && (
                        <Menu
                            as="div"
                            className="relative ml-3 inline-block float-right mr-5 mt-2"
                        >
                            <div>
                            ...
                            <Transition
                                ...
                                leaveTo="transform opacity-0 scale-95"
                            >
                                <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md dark:bg-slate-700 bg-white shadow-lg ring-1 ring-indigo ring-opacity-5 focus:outline-none">
                                                ...
                                                {({ active }) =>
                                                    item.route ? (
                                                        <Link
                                                            ...
                                                            className={classNames(
                                                                active
                                                                    ? "bg-indigo-100 text-indigo-900 dark:bg-slate-500 dark:text-indigo-900"
                                                        ...
                                                        >
                                                            <img
                                                                src={`/${item.iconFname}`}
                                                                className="h-6 w-6 inline-block mr-2 text-indigo-700"
                                                            />
                                                            {item.name}
                                                        </Link>
                                                    ) : (
